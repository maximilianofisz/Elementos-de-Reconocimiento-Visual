...
def extraerSIFTDataSet(images, labels):
  ...
  for i, (img, label) in enumerate(zip(images, labels)):
    ...
    keypoints, descriptors = extraerSIFTImagen(img)
...
def build_visual_vocabulary(descriptors,n_visual_words):

    print(f"Construyendo vocabulario visual mini batch kmeans con {n_visual_words} palabras visuales...")

    model = MiniBatchKMeans(n_clusters=n_visual_words,random_state=42, batch_size=512)
...
def crossval(images, labels, file_names, vocab_options,n_subsets=3):
    resultados = []

    # ORB
    keypoints, descriptors, stats = extraerSIFTDataSet(images, labels)
    ...
    for n_visual_words in vocab_options:
        ...
        for i in range(n_subsets):
            ...
            vocab = build_visual_vocabulary(train_desc_matrix, n_visual_words)
...
def best_result(results):
  ...
  # Header
  print("="*108)
  print(f"{'n_visual_words':>15} | {'acc_prom':>10} | {'f1_prom':>8} | {'t_total(s)':>10} | {'t/img(s)':>8}")
  print("="*108)

  for res in sorted_results:
    print(f"{res['n_visual_words']:15} | " f"{res['accuracy_promedio']:.4f}   | {res['f1_promedio']:.4f}  | " f"{res['total_time']:.2f}     | {res['avg_time']:.4f}")
    print("="*108)
