...
def build_visual_vocabulary(descriptors, n_keypoints, n_visual_words, sample_size):

    print(f"Construyendo vocabulario visual con {n_keypoints} keypoints y {n_visual_words} palabras visuales...")

    model = CLARA(n_clusters=n_visual_words, metric='hamming', 
                  n_sampling=sample_size, n_sampling_iter=10)
...
def crossval(images, labels, file_names, keypoint_options, vocab_options, sample_options,n_subsets=3):
    resultados = []
    ...
    for n_keypoints in keypoint_options:
        ...
        for n_visual_words in vocab_options:
            ...
            for sample_size in sample_options:
                ...
                print(f"\nEvaluando: {n_keypoints=}, {n_visual_words=}, {sample_size=}")
                ...
                for i in range(n_subsets):
                    ...
                    vocab = build_visual_vocabulary(train_desc_matrix, n_keypoints, n_visual_words, sample_size)
...
def best_result(results):
  ...
  for res in sorted_results:
    print(f"{res['n_keypoints']:12} | {res['n_visual_words']:15} | {res['sample_size']:12} | " f"{res['accuracy_promedio']:.4f}   | {res['f1_promedio']:.4f}  | " f"{res['total_time']:.2f}     | {res['avg_time']:.4f}")
    print("="*108)
